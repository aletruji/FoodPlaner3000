        ...
        Box(
            ...
            LazyColumn(modifier = Modifier.fillMaxSize()) {
                itemsIndexed(
                    ...
                    var itemToEdit by remember { mutableStateOf<DishItem?>(null) }
                    var showDeleteDialog by remember { mutableStateOf(false) }
                    var itemToDelete by remember { mutableStateOf<DishItem?>(null) }
                    ...
                    if (dismissState.isDismissed(DismissDirection.EndToStart) && !isHandled) {
                        ...
                        scope.launch {
                            ...
                            if (result == SnackbarResult.ActionPerformed) {
                                val restoredItem = DishItem(Item(removedItemName, removedQuantity, removedItemUnit), removedQuantity, removedItemUnit)
                    ...
                    if (showDeleteDialog && itemToDelete != null) {
                        AlertDialog(
                            ...
                            confirmButton = {
                                TextButton(onClick = {
                                    val itemIndex = viewModel.items.indexOf(itemToDelete!!)
                                    if (itemIndex != -1) {
                                        viewModel.removeItem(itemIndex)
                                    }
