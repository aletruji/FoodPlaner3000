        ...
        text = {
            Column {
                ...
                items.forEachIndexed { index, dishItem ->
                    Text(
                        text = "- ${dishItem.item.name}: ${dishItem.quantity} ${dishItem.unit}",
                        modifier = Modifier
                            .fillMaxWidth()
                            .clickable {
                                // Beim Klicken: Werte zurück in die Eingabefelder setzen
                                newItemName = dishItem.item.name
                                newItemQuantity = dishItem.quantity.toString()
                                selectedUnit = dishItem.unit
                                // Entferne das bearbeitete Item aus der Liste, um es nachher neu hinzuzufügen
